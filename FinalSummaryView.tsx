...
}> = ({ settings, lang, cart, details, total, onBack, onConfirm, isSubmitting }) => {
  ...
  return (
    <div className={`h-full flex flex-col animate-scale-up ${isDark ? 'bg-[#0F172A]' : 'bg-[#F9FAFB]'}`}>
      <header className={`flex-shrink-0 px-4 pb-4 pt-[calc(0.75rem+env(safe-area-inset-top))] flex items-center space-x-3 border-b ${isDark ? 'bg-[#1E293B] border-white/5' : 'bg-white border-gray-100'}`}>
        ...
        <h2 className={`text-2xl font-black font-oswald uppercase tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{lang === 'EN' ? 'Review' : 'समीक्षा'}</h2>
      </header>
      <div className="flex-1 overflow-y-auto p-4 space-y-6 no-scrollbar pb-64">
        <section className="space-y-3">
          <h3 className="text-[9px] font-black uppercase tracking-widest opacity-40">{lang === 'EN' ? 'Contact Info' : 'संपर्क जानकारी'}</h3>
        ...
        <section className="space-y-3 pb-20">
          <h3 className="text-[9px] font-black uppercase tracking-widest opacity-40 px-1">{lang === 'EN' ? 'Selected Items' : 'चयनित आइटम'}</h3>
      ...
      <div className={`flex-shrink-0 px-6 pb-10 pt-6 border-t rounded-t-[2.5rem] shadow-[0_-20px_60px_rgba(0,0,0,0.4)] ${isDark ? 'bg-[#1E293B] border-white/5' : 'bg-white'}`}>
        <div className="flex justify-between items-end mb-5">
          <span className="text-slate-400 font-black uppercase text-[10px] tracking-widest">{lang === 'EN' ? 'Grand Total' : 'कुल योग'}</span>
